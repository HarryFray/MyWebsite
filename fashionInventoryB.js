...
*/
averageShoeCost(inventory)

function averageShoeCost(inventory){
  var expected = {
    'designers': []
  }
  inventory.forEach(function(element){
    expected['designers'].push({'name':element.name, 'averagePrice':computeAverage(element)})
...
function computeAverage(element){
  var sum = 0
  var count = 0
  element.shoes.forEach(function(obj){
    sum += obj.price
    ++count
  })
  return sum / count
